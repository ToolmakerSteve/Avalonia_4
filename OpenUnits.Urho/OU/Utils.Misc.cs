...
namespace OU
{
	...
	public static partial class Utils
	{
		...
		//        ///     ''' <returns></returns>
		//        public static IEnumerable<Dist2D> BackwardsNoDup(this IList<Dist2D> list1, Dist2D? previousT, double tolerance = EpsilonForOne)
		//        {
		...
		//        ///     ''' <param name="tolerance"></param>
		//        ///     ''' <returns></returns>
		//        public static IEnumerable<Dist2D> ForwardNoDup(this IList<Dist2D> list1, Dist2D? previousT, double tolerance = EpsilonForOne)
		//        {
		//            int iLast = LastIndex(list1);
		...
		//        }
		//        // A reasonable "toleranceFraction" is ScaledToleranceDouble. Larger if multiple operations may accumulate error.
		//        public static bool NearlyEquals_ScaledTolerance(this Dist2D p1, Dist2D p2, double toleranceFraction)
		//        {
		...
		//        public static void Swap(ref Dist2D ptdFirst, ref Dist2D ptdSecond)
		//        {
		//            Dist2D tmp = ptdFirst;
		//            ptdFirst = ptdSecond;
		//            ptdSecond = tmp;
		...
		//        public static void Swap(ref Dist3D ptdFirst, ref Dist3D ptdSecond)
		//        {
		//            Dist3D tmp = ptdFirst;
		//            ptdFirst = ptdSecond;
		//            ptdSecond = tmp;
		...
		public static DistD CalcDistance2(DistD x1, DistD y1, DistD x2, DistD y2)
		{
			return DistanceSquared2D(x1, y1, x2, y2).Sqrt();
		}

		public static DistD CalcDistance2(Dist2D p1, Dist2D p2)
		{
			return DistanceSquared2D(p1.X, p1.Y, p2.X, p2.Y).Sqrt();
		}

		public static DistD CalcDistance2(Dist3D p1, Dist3D p2)
		{
		...
		{
			// Equivalent.
			return DistD.FromDefaultUnits(a.Value + b.Value) / 0.5;
			//return Lerp(a, b, 0.5);
		}
		public static Vec3 Average(Vec3 a, Vec3 b)
		{
			return new Vec3(0.5f * (a.X + b.X), 0.5f * (a.Y + b.Y), 0.5f * (a.Z + b.Z));
		}
		...
		public static Vec3 Lerp2D(Vec2 xyWgt, Vec3 X0Y0, Vec3 X1Y0, Vec3 X0Y1, Vec3 X1Y1)
		{
			Vec3 xY0 = Lerp(X0Y0, X1Y0, xyWgt.X);
			Vec3 xY1 = Lerp(X0Y1, X1Y1, xyWgt.X);

			Vec3 xy = Lerp(xY0, xY1, xyWgt.Y);
			return xy;
		}

		public static Vec2 Lerp2D(Vec2 xyWgt, Vec2 X0Y0, Vec2 X1Y0, Vec2 X0Y1, Vec2 X1Y1)
		{
			Vec2 xY0 = Lerp(X0Y0, X1Y0, xyWgt.X);
			Vec2 xY1 = Lerp(X0Y1, X1Y1, xyWgt.X);

			Vec2 xy = Lerp(xY0, xY1, xyWgt.Y);
			return xy;
		}
		...
		public static float Lerp2D(Vec2 xyWgt, float ZAtX0Y0, float ZAtX1Y0, float ZAtX0Y1, float ZAtX1Y1)
		{
		...
		//public static Dist2D InverseLerp2D(Dist2D xy, Dist2D X0Y0, Dist2D X1Y0, Dist2D X0Y1, Dist2D X1Y1)
		//{
		//    // ----> Main Work, Pass 1 <----
		//    Dist2D xyWgt = _InverseLerp2D_Pass1(xy, X0Y0, X1Y0, X0Y1, X1Y1);

		//    // Verify
		//    Dist2D xy_verify = Lerp2D(xyWgt, X0Y0, X1Y0, X0Y1, X1Y1);
		...
		//private static Dist2D _InverseLerp2D_PassN(Dist2D xyWgt1, Dist2D xy, Dist2D X0Y0, Dist2D X1Y0, Dist2D X0Y1, Dist2D X1Y1)
		//{
		...
		//    Dist2D XguessY0 = Lerp(X0Y0, X1Y0, xWgt);
		//    Dist2D XguessY1 = Lerp(X0Y1, X1Y1, xWgt);
		//    double yWgt2;
		...
		//    Dist2D Xguess_closest = ClosestPointOnLine2D_AndT(xy, XguessY0, XguessY1, yWgt2);

		//    double yWgt = xyWgt1.Y;
		//    Dist2D X0Yguess = Lerp(X0Y0, X0Y1, yWgt);
		//    Dist2D X1Yguess = Lerp(X1Y0, X1Y1, yWgt);
		//    double xWgt2;
		//    // y params => xWgt2 ("x" is not a typo).
		//    Dist2D Yguess_closest = ClosestPointOnLine2D_AndT(xy, X0Yguess, X1Yguess, xWgt2);
		...
		//private static Dist2D _InverseLerp2D_Pass1(Dist2D xy, Dist2D X0Y0, Dist2D X1Y0, Dist2D X0Y1, Dist2D X1Y1)
		//{
		//    double wx0;
		...
		//public static Dist2D InverseLerp2D_A(Dist2D xy, Dist2D X0Y0, Dist2D X1Y0, Dist2D X0Y1, Dist2D X1Y1)
		//{
		//    double x = xy.X;
		...
		public static string AsNDigits(Vec3 vec, int nDigits)
		{
			return string.Format("({0}, {1}, {2})", RoundN(vec.X, nDigits), RoundN(vec.Y, nDigits), RoundN(vec.Z, nDigits));
		}

		public static string ShortString(Vec3 vec)
		{
		...
		public static string AsShortString(Vec3 v)
		{
		...
		{
			return DistD.FromDefaultUnits(Average3(a.Value, b.Value, c.Value));
		}
		...
		//            public PolarPoint(Dist2D pt) : this(pt.X.Value, pt.Y.Value)
		//            {
		//            }
		...
		//            public static PolarPoint operator +(PolarPoint p1, Dist2D p2)
		//            {
		//                Dist2D sum = p1.AsPoint2D() + p2;
		//                return new PolarPoint(sum);
		//            }

		//            public Dist2D AsPoint2D()
		//            {
		//                // TODO: How will GeoContext deal with units?
		//                Dist2D pt = new Dist2D(Length * Math.Cos(Angle), Length * Math.Sin(Angle), null);
		//                return pt;
		//            }
		//        }
